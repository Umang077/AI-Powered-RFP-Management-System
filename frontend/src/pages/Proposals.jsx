...
//       <div className="card">
//         <label>Enter RFP ID to view proposals</label>
//         <input value={rfpId} onChange={e => setRfpId(e.target.value)} />
//         <button onClick={fetch} disabled={loading}>{loading ? "Loading..." : "Load Proposals"}</button>
...
//       {proposals.length === 0 ? (
//         <div className="small">No proposals found. After sending RFPs, trigger backend email receive or wait for vendor replies.</div>
//       ) : (
//         <div className="grid">
//           {proposals.map(p => (
//             <div key={p._id} className="card">
//               <div className="small">Vendor: {p.vendorId || "unknown"}</div>
//               <pre style={{ whiteSpace: "pre-wrap" }}>{JSON.stringify(p.parsedProposal || {}, null, 2)}</pre>
//               <div className="small">Raw email (truncated):</div>
//               <pre style={{ maxHeight: 120, overflow: "auto", whiteSpace: "pre-wrap" }}>{(p.rawEmail || "").slice(0, 2000)}</pre>
//             </div>
//           ))}
//         </div>
//       )}
